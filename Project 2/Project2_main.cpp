...
int		Ranf( int, int );
float 		fx,fy,fz;								//fx,fy,fz declared
...
//		#pragma omp parallel for reduction(+:fx,fy,fz)/*, schedule(static)*/, schedule(dynamic)  				//coarse-grained parallelism
		for( int i = 0; i < NUMBODIES; i++ )
		{
			float fx = 0.;
			float fy = 0.;
...
//			#pragma omp parallel for reduction(+:fx,fy,fz)/*, schedule(static)*/, schedule(dynamic)				//Fine-grained parallelism
	...
	float performance = ((float)(NUMBODIES*NUMBODIES*NUMSTEPS)/(time1-time0)/1000000.);	
